...
class LatentDocOPTForCausalLM(OPTForCausalLM):
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.vision_encoder = build_resnet152()

        self.mm_projector = nn.Linear(2048, self.config.hidden_size)
    ...
    def init_multimodal_module(self, tokenizer, multimodal_cfg=None):
        ...
        if self.training:
            ...
            multimodal_cfg.img_patch_token_id = tokenizer.convert_tokens_to_ids(multimodal_cfg.special_tokens.img_patch_token)
            multimodal_cfg.im_start_token_id = tokenizer.convert_tokens_to_ids(multimodal_cfg.special_tokens.im_start_token)
            multimodal_cfg.im_end_token_id = tokenizer.convert_tokens_to_ids(multimodal_cfg.special_tokens.im_end_token)
            multimodal_cfg.img_start_token_id = tokenizer.convert_tokens_to_ids(multimodal_cfg.special_tokens.img_start_token)
            multimodal_cfg.img_end_token_id = tokenizer.convert_tokens_to_ids(multimodal_cfg.special_tokens.img_end_token)

            self.config.multimodal_cfg = multimodal_cfg

            self._resize_embedding(tokenizer)
            self._expand_max_length()
        
        else:
            multimodal_cfg = self.config.multimodal_cfg
            self.config.multimodal_cfg = edict(multimodal_cfg)
          

        return tokenizer, multimodal_cfg

    def _expand_max_length(self,):
        if self.config.multimodal_cfg.model_max_length > self.config.max_position_embeddings:
            ...
            # interpolate
            new_pe_weight = interpolate_positional_encoding(original_pe_weight, self.config.multimodal_cfg.model_max_length + 2)

            # create a new Embedding
            new_embed_positoins = OPTLearnedPositionalEmbedding(self.config.multimodal_cfg.model_max_length, dim)
            ...
            logging.warning(f'the pe of opt model is interpolated from {self.config.max_position_embeddings} to {self.config.multimodal_cfg.model_max_length}')
            self.config.max_position_embeddings = self.config.multimodal_cfg.model_max_length
            self.model.decoder.max_target_positions = self.config.multimodal_cfg.model_max_length
        pass
    ...
    def multimodal_process(self, input_ids, input_embeddings, img_features):
        ...
        for cur_input_ids, cur_input_embeds, cur_image_features in zip(input_ids, input_embeddings, img_features):
            ...
            if (cur_input_ids == self.config.multimodal_cfg.img_patch_token_id).sum() == 0:
                # print(cur_input_ids)
            ...
            # check the input
            if (cur_input_ids == self.config.multimodal_cfg.im_start_token_id).sum() != (cur_input_ids == self.config.multimodal_cfg.im_end_token_id).sum() and self.training:
                    raise ValueError("The number of input message start tokens and input message end tokens should be the same.")


            if (cur_input_ids == self.config.multimodal_cfg.img_start_token_id).sum() != (cur_input_ids == self.config.multimodal_cfg.img_end_token_id).sum() :
                    raise ValueError("The number of image start tokens and image end tokens should be the same.")


            image_start_tokens = torch.where(cur_input_ids == self.config.multimodal_cfg.img_start_token_id)[0]
            ...
            for image_start_token_pos in image_start_tokens:
                ...
                # check the input
                if cur_input_ids[image_start_token_pos + num_patches + 1] != self.config.multimodal_cfg.img_end_token_id:
...
def build_model():
    ...
    tokenizer, img_processor = model.init_multimodal_module(tokenizer, multimodal_cfg)
...
def check_model_parameters():
    from latentdoc.model.vision_encoder.sam import build_sam_vit_b
    model, tokenizer, img_processor = build_model()
    model_state_dict = model.state_dict()
    img_processor, vision_encoder = build_resnet152_and_img_processor()
    ...
    # print(model_state_dict['vision_encoder.bn1.bias'])
    

    
    # print(vision_state_dict['bn1.bias'])
