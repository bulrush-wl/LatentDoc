...
class LatentDocTrainer_ae(Trainer):
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            tr_loss -= tr_loss
            logs['ae_loss']=round(float(self.output_my['ae_loss']),4) if 'ae_loss'in self.output_my.keys() else None
            logs['l2_loss']=round(float(self.output_my['l2_loss']),4) if 'l2_loss'in self.output_my.keys() else None
            logs['gpp_loss']=round(float(self.output_my['gpp_loss']),4) if 'gpp_loss'in self.output_my.keys() else None
            logs['my_loss']=round(float(self.output_my['loss']),4)  if 'loss'in self.output_my.keys() else None
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        super(LatentDocTrainer_ae, self)._save(output_dir, state_dict)
