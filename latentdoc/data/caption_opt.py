...
class CaptionDataset(BaseDataset):
    ...
    def __init__(self, datasets, tokenizer, img_processor, multimodal_cfg):
        super(CaptionDataset, self).__init__(datasets, tokenizer, img_processor, multimodal_cfg)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data = copy.deepcopy(self.list_data_dict[i])  # 可能后续会对数据做些改变，但又不希望修改原始的数据
    ...
    def multimodal_processor(self, sources):
        ...
        for source in sources:
            if self.multimodal_cfg['sep_image_conv_front']:
                assert DEFAULT_IMAGE_TOKEN in source[0]['value']  # 图片必须在第一次对话时就提供
